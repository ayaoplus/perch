...
这是 **v1 早期实现阶段** 的仓库。**Step 1 已全部收尾**,进入 Step 2:
...
- `scripts/` 含 `spike-list.mjs`(S1.4 review gate)和 `spike-profile.mjs`(S1.6 review gate),均已由外部 reviewer / Codex 验证通过
- 下一步进入 **Step 2 — topic 配置加载 + `/perch collect` + raw 落盘**(S2.1–S2.5)
...
- `scripts/`:review gate / spike 脚本。已有 `spike-list.mjs`(S1.4)与 `spike-profile.mjs`(S1.6)
...
2. 优先遵循 `docs/TASKS.md` 的当前步骤。**Step 1 已收尾**;当前重点是 **Step 2 — topic 配置 + `/perch collect` 端到端**(S2.1–S2.5)。
...
6. Step 2 的 collect 层按 DESIGN §2.1 **Fetch / Business / Tool** 三层组织职责:fetch 保持原样(不排序 / 不做时间窗 / 不过滤 pinned),业务语义(时间窗增量、pinned 处理、跨次去重)只落在 collect 层,不要把这些逻辑倒回 Fetch 层。
7. 仓库还小,Step 1 主链路已成形。优先搭 Step 2 collect 调用链,不要为了“未来扩展”过度抽象。
...
1. **Step 2 — Topic 配置加载 + `/perch collect` + raw 落盘**(S2.1–S2.5)
2. Step 3 — Daily Wiki 生成(morning/noon/evening)
3. Step 4 — 月度 rotate 脚本
