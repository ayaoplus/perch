...
- `lib/` 是 Fetch + Tool 层:CDP 栈、X 抓取(从 redux store 读 timeline,长推 / repost / metrics 自带)、normalize(dedup 聚合)、topic 加载、wiki 路径 + section 级 upsert、article 缓存、rotate
- `scripts/` 是 Business 层入口:collect / report / rotate / fetch-article / new-topic / wiki-write + spike 脚本
- `templates/topics/<slug>/` 每个 topic 的 SCHEMA.md + 时段 prompt
- `config.example.json` 是提交到 git 的占位模板,`config.json`(本地、gitignore)指定 default_topic / timezone / 各 topic 的 path 和 templates_dir
...
- `config.json`(gitignore):本地运行时配置,默认 topic、topic 路径、rotate 配置
- `lib/`:Fetch 层(x-fetcher / CDP 栈 / x-adapter)+ Tool 层(normalize / topic / wiki / article-cache / rotate)
- `scripts/`:Business 层入口(collect / report / rotate / fetch-article / new-topic / wiki-write)+ review gate spike 脚本
...
- **优先用 `report now`**,时区 / wrap / 归属日期 / window 全部自动解析
...
- **永远不会出现反向窗口或未来窗口** — 前一版的两个边界 bug 都已消除
...
- **wiki 写入必须走 `{WIKI_WRITE_CMD}` heredoc pipe,不能 Write 工具直接覆盖 `{WIKI_PATH}`** — 当日 wiki 是共享文件,直接覆盖会抹掉其他 slot 的 section。`wiki-write.mjs` 会自动加 `## slot: <name>` 外层标题 + 按 start_hour 重排 sections,Claude 只负责生成自己那段 markdown
...
v1 主链路全部实现,仓库内置一个示例 topic(`ai-radar`,SCHEMA 的 X list ID 是占位符)。当前重心:

1. 让实际 topic 的产出打磨到每天真用(收集反馈、调时段 prompt、检验归档 / wrap / 全局重排)
