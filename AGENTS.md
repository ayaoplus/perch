...
Agent 如需新建 topic,**首选 `scripts/new-topic.mjs --from-json <spec>`**(非交互、有校验、不覆盖已有配置);`scaffoldTopic` / `validateTopicSpec` 也可以直接 import。完整字段定义、端到端流程、常见坑见 `docs/TOPIC_AUTHORING.md`。不要手工拼 SCHEMA.md + config.json,容易漏校验。
...
2. 稳定后,加**第二个 topic**(比如 crypto)验证配置化真的 work — 用 `scripts/new-topic.mjs --from-json` 一把过
