...
│   ├── normalize.mjs           # perch: tweet 原始对象 → raw markdown block + 去重排序辅助
│   ├── topic.mjs               # perch: Topic 配置加载(读 config.json + SCHEMA.md frontmatter)
│   └── rotate.mjs              # perch: 月度归档(Step 4 实现)
├── scripts/                    # 可执行脚本(review gate + 业务入口)
│   ├── spike-list.mjs          # S1.4 list review gate
│   ├── spike-profile.mjs       # S1.6 profile review gate
│   └── collect.mjs             # /perch collect 的 Business 层入口
├── sources/                    # 采集插件(可扩展端之一)
│   ├── x-list.md
│   └── x-user.md
...
└── templates/topics/<slug>/    # 每个 topic 的逻辑配置(随 perch 仓库版本化)
    ...
    ├── morning.md              # 时段报告 prompt(Step 3 展开)
    ├── noon.md
    └── evening.md
```
...
| 资产 | 关系 |
|---|---|
| `ai-radar`(前身项目) | 保留但冻结。其 templates 和数据目录在 perch 里作为**首个 Topic**(slug `ai-radar`)继续存在 |
...
| 2 | Skill 骨架 + config + `ai-radar` topic 的 SCHEMA + 运行时配置 | `perch collect` 跑通,raw 落到新结构 |
