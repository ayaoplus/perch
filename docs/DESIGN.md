...
| 层 | 位置 | 做什么 | **不**做什么 |
|---|---|---|---|
| **Fetch** | `lib/x-fetcher.mjs` · `lib/x-adapter.mjs` | 打开 X 页面返回原始 tweet,2-pass 稳定性吸收 lazy DOM,`isTruncated` 长推文自动 hydrate,识别 `socialContext` 区分纯 RT vs quote | 不排序、不做时间窗过滤、不识别 pinned |
| **Business** | `scripts/collect.mjs` · `scripts/report.mjs` · `scripts/rotate.mjs` · `scripts/fetch-article.mjs` · `scripts/new-topic.mjs` | 编排业务流:generous limit + 跨次去重 + 按时间排序追加 · 时段 prompt 替换 · 窗口计算 · 月度归档 · 按需 article 深抓 · topic 脚手架 | 不直接碰 CDP / 页面 DOM |
| **Tool** | `lib/normalize.mjs` · `lib/topic.mjs` · `lib/wiki.mjs` · `lib/rotate.mjs` · `lib/article-cache.mjs` | 可组合原子:`formatTweet` / `dedupTweets`(聚合 `__via` + `repostedBy`)/ `sortTweetsByTime` / `readExistingIds` / `loadTopic` / 路径辅助 / article 缓存读写 | 不做业务流编排 |

**为什么这么分**:实证例子是 profile 页面的 pinned tweet。`fetchXProfile(.., {limit:5})` 顶部可能是钉着的老推文,"顺序错"只是表象,**本质是 pinned 挤占一个位置导致最老应收推文漏采**。如果修法是"在 Fetch 层默认按时间排 + 过滤 pinned",就把业务语义污染到底层 API,将来"想保留 pinned"或"想要 DOM 原序"的场景就得反向 opt-out。反过来保持 Fetch 干净,业务层按场景自行组合,扩展性和可测试性都更好。

**为什么 hydrate truncated 归到 Fetch 层**:X 的 UI 对 >280 字长推默认截断,tweetText 里只有截断版。这是**数据完整性兜底,不是业务决策**——Business 层不应该感知"哪些需要二抓"。同理 socialContext(纯 RT 识别)是 DOM 选择器层的事,Fetch 层一并处理。这两条跟 2-pass stabilize 是同级概念:**Fetch 层的 robustness,给上层一个"完整且正确的原始数据"的保证**。

**真实业务场景(collect 管线)**:

```
fetchXList/Profile(generous limit,例如 80~200)   # 每个 source 独立拉
  → (Fetch 内) 2-pass stabilize + hydrate truncated # 完整正文 + 纯 RT 识别
  → normalize.dedupTweets                          # 跨 source 合并 + 聚合 __via / repostedBy
  → readExistingIds 对比当日 raw 文件,过滤已存在
  → normalize.sortTweetsByTime                     # 写盘前的最终时间倒序
  → formatTweet 追加写盘(block 标 `via:` / `🔁 reposted by:`)
```

预期跑频 3~4 次/天(多 topic 可并行排期,见 §3.1)。**不需要显式"时间窗 / lastRunTime"状态** — ID 去重(`readExistingIds` + `dedupTweets`)已完全覆盖去重需求,`fetch_limit` 的 generous 取值自然限定了"只看最近一批"。generous limit 的真正作用是**给 pinned / DOM 抖动 / 当日高频账号留余量**,让下游 ID 去重兜底而不是让 limit 卡边界。
...
- `today` — 从今日 00:00 到触发时刻,累积视角(适合"今天讨论最多的 X"这类统计型问题)
- `since_prev` — 只覆盖上一 slot 的 start_hour 到触发时刻,增量视角(适合"这段时间新出什么"类问题);首个 slot 自动 fallback 为 today,避免跨昨日 raw 的复杂度

window 不影响 raw 物理结构(仍是一日一文件),只通过 `{WINDOW_*}` 占位符注入到 prompt,让 Claude 自行按 `MM-DD HH:MM` 时间戳过滤 block。

**Topic 脚手架**:`scripts/new-topic.mjs` 提供两种创建方式:交互向导(人用)和 `--from-json`(agent 用)。后者还暴露了 `scaffoldTopic` / `validateTopicSpec` / `renderSchemaMd` / `renderSlotPrompt` 四个可 import 的纯函数。详见 `docs/TOPIC_AUTHORING.md`。
...
│   ├── x-adapter.mjs           # X list/profile/status/longform 提取逻辑 + socialContext 识别
│   ├── x-fetcher.mjs           # 上述组合,对外暴露 fetchXList / fetchXProfile(2-pass + hydrate)
...
- **长推完整性**:`isTruncated=true` 的长推在 collect 阶段被自动 hydrate(单独访问 status URL 抓完整 tweetText),写进 raw 的 `type:` 行带 `hydrated` 标志
- **转发信号聚合**:
...
- **block 格式**:
  ...
  type: tweet · hydrated             (hydrated 仅在长推被补齐后出现;truncated 保留在失败时)
...
X adapter 的选择器基于 `data-testid`(相对稳定),但 X 产品改版时可能失效。list / profile / search / status / longform 五种 page type 各自有不同 selector 集。

**缓解**:`scripts/spike-list.mjs` 和 `scripts/spike-profile.mjs` 作为可重放 gate 能在改版后立刻暴露断裂;补 selector 尽量不动 x-adapter 内部代码(保持 replacability)。
...
### R5. 单次 collect 的 lazy-DOM 抖动

X timeline 页面异步渲染 — 首次 DOM extract 可能只看到 cache,最新推文几秒后才 append 到顶部。若单次 extract 就信号盘,会漏最新一批推文。

**缓解**:`lib/x-fetcher.mjs` 默认 **2-pass extract**(warmup → extract₁ → 3s stabilize → extract₂ → 按 statusId 合并),Fetch 层吸收 jitter,Business 层不用感知。spike 脚本或单次探测可 `options.stabilize=false` opt-out。
...
- Quote tweet 的独立 hydrate(去掉人为 140 截断后,若 X UI 截断的 quote 成为痛点再补)
