...
| 层 | 位置 | 做什么 |
|---|---|---|
| **Adapter** | `lib/x-fetcher.mjs` · `lib/x-adapter.mjs` · CDP 栈 | 和外部世界打交道:读 X redux store,CDP 控制浏览器 |
...
### R3. Skill 模式的不健壮
report 依赖"当前 Claude 会话会读 stdout 的 prompt 并接棒"。脱离会话静默失败。
**缓解**:v3.x 引入 `lib/llm.mjs::complete()`(direct API 模式),同一份 method、同一份 prompt,仅 LLM 调用一步换实现。
...
| 8 | LLM Direct 模式(`lib/llm.mjs::complete`) | ⏸ 留 v3.x |
| 9 | SCHEMA `schedule` 字段 + 通用 runner | ⏸ 留 v3.x |
...
- LLM Direct 模式(留接口位置)
- Schedule 自动驱动(v3 哲学就是不做调度,只在 LLM Direct 落地后再考虑)
