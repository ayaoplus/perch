...
| 层 | 位置 | 做什么 | **不**做什么 |
|---|---|---|---|
| **Fetch** | `lib/x-fetcher.mjs` | 打开 X 页面,按 DOM 顺序返回 N 条原始 tweet(含 2-pass 稳定性吸收 lazy DOM) | 不排序、不做时间窗过滤、不识别 pinned |
| **Business** | `scripts/collect.mjs` · `scripts/report.mjs` · `scripts/rotate.mjs` | 编排业务流:generous limit + 跨次去重 + 按时间排序追加 · 时段 prompt 替换 · 月度归档 | 不直接碰 CDP / 页面 DOM |
| **Tool** | `lib/normalize.mjs` · `lib/topic.mjs` · `lib/wiki.mjs` · `lib/rotate.mjs` | 可组合原子:`formatTweet` / `dedupTweets` / `sortTweetsByTime` / `readExistingIds` / `loadTopic` / 路径辅助 | 不做业务流编排 |
...
fetchXList/Profile(generous limit,例如 80~200)   # 每个 source 独立拉
  → normalize.dedupTweets                          # 跨 source 合并后按 statusId 去重
  → readExistingIds 对比当日 raw 文件,过滤已存在
  → normalize.sortTweetsByTime                     # 写盘前的最终时间倒序
  → formatTweet 追加写盘(block 标 `via: <slug>`)
```

预期跑频 3~4 次/天。**不需要显式"时间窗 / lastRunTime"状态** — ID 去重(`readExistingIds` + `dedupTweets`)已完全覆盖去重需求,`fetch_limit` 的 generous 取值自然限定了"只看最近一批"。generous limit 的真正作用是**给 pinned / DOM 抖动 / 当日高频账号留余量**,让下游 ID 去重兜底而不是让 limit 卡边界。
...
**时段槽位(slots)是 Topic 级配置**:在 SCHEMA.md 的 frontmatter 里用 `slots: [{name, start_hour}]` 定义。不同 Topic 可以有不同数量、不同边界的时段(如新闻类早高峰 4 槽、市场类晚高峰 3 槽)。缺省时 fallback 到 `morning@5 / noon@12 / evening@18` 保持向后兼容。每个 slot name 对应同目录的 `<name>.md` 模板文件。
...
- 活跃库 = 只当月
- 每月 1 号 rotate 上月 raw/daily + wiki/daily 到 `archive/YYYY-MM/`
...
│   ├── x-adapter.mjs           # X list/profile/status/longform 提取逻辑
│   ├── x-fetcher.mjs           # 上述组合,对外暴露 fetchXList / fetchXProfile(含 2-pass 稳定性)
│   ├── normalize.mjs           # tweet 对象 → raw markdown block + 去重 / 排序辅助
...
**说明**:
- Topic 数据目录**只装运行时产物**(raw / wiki / summaries / archive),不入 git。路径由 config.json 指定,可以放在 iCloud / Obsidian 等同步盘
...
- **一天一个文件**,时间倒序(最新在上)
- **多 source 合并**:同 Topic 下配置的多个 source(多 list / 多 profile)写进同一天文件,block 内用 `via: <source-slug>` 标注来源,单 source 场景可省略
- **去重粒度 = tweet ID**,只扫每个 block 标题行的 `/status/(\d+)`(避免把 quote/link 行里的 ID 误算为顶层已存在)
- **block 格式**:
  ```
  ## @handle (Name) · MM-DD HH:MM · [source](url)
  type: tweet
  via: <source-slug>                (多 source 时出现,值来自 Topic SCHEMA.md 的 source slug)

  推文正文

  📊 N RT · N 💬 · N ❤️ · Nx views
  🖼️ images · video · article       (可选,有媒体才出现)
  🔗 quote: @handle — text [url]    (可选,有引用才出现)
  🔗 link: <external-url>           (可选,有卡片才出现)
...
| 3 | Daily Wiki 生成(morning / noon / evening) | ✅ 完成(Skill 模式,cron 化留 v2) |
| 4 | 月度 rotate 脚本(`--dry-run` + 幂等) | ✅ 完成(真月末归档留 live gate) |
| 5 | 加第二个 topic 验证配置化 | ⏳ 待首个 topic 打磨到每天真用后再挑 |
...
- `summaries.md` 的月度切分归档(rotate 只动 raw/daily + wiki/daily,v2 再补 summaries 切分)
