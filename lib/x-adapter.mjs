...
// List timeline 从 X redux store 直接读 —— X 页面自己调 ListLatestTweetsTimeline GraphQL
// 把结果塞到 state.urt[`listTweets-GraphQL-${listId}-latest`],entries 通常首轮就 80+ 条。
// 比 DOM 爬虫完整 + idempotent + 不受虚拟列表/tab throttle 影响。
//
...
const BROWSER_LIST_API_JS = String.raw`
...
async function fetchListTimelineItems(options) {
  ...
  if (!listId) return { items: [], error: 'no_list_id', strategy: 'none' };

  const context = getRuntimeContext();
  const store = context && context.store;
  if (!store) return { items: [], error: 'store_unavailable', strategy: 'none' };
  ...
  while (Date.now() - start < MAX_WAIT_MS) {
    const tl = getCurrentListTimeline(store, listId);
  ...
  const timeline = getCurrentListTimeline(store, listId);
  if (!timeline) return { items: [], error: 'timeline_not_in_store', strategy: 'store' };
...
// minArticles=10:X 的 list/profile 第一批通常 mount 20-30 条,5 太低 — 只挂 4-5 条就放行,
// collectTimelineItems 首轮 scroll 根本不够一屏,触发不了 IntersectionObserver 加载下一批,
// 会误判为 stagnant 而提前 break(实测 Pass 1 塌在 4 条就出)。
async function waitForTimelineSettled(proxy, targetId, { maxWait = 20000, minArticles = 10, pollInterval = 500 } = {}) {
  const start = Date.now();
...
async function collectTimelineItems(proxy, targetId, limit, {
  ...
  const debug = process.env.PERCH_DEBUG_COLLECT === '1';
  const dbg = (msg) => { if (debug) console.error(`[collect-debug] ${msg}`); };

  let stagnant = 0;
  // added=0 且没 loading / retry 时,不立刻计 stagnant — X 的 lazy load 有批次空窗,
  // 先给一次长等 + 再滚的兜底,用尽这几次才真正进入 stagnant 计数。
  let extraWaitsLeft = 2;
  for (let attempt = 0; attempt < maxScrollAttempts; attempt++) {
    ...
    dbg(`attempt=${attempt} domArticles=${currentArr.length} added=${added} totalItems=${items.length} stagnant=${stagnant}`);

    if (items.length >= limit) { dbg(`break: reached limit`); break; }
    ...
    if (added === 0 && items.length > 0) {
      ...
      if (clickedRetry) {
        dbg(`clicked retry button`);
        stagnant = 0;
        await sleep(scrollDelay);
        continue;
      }

      // 没 loading、没 retry,也不一定真的到底 — X 经常在两个批次之间有短暂空窗。
      // 先用 extraWaitsLeft 做一次"长等 + 再滚",兜底用尽才真正计 stagnant。
      if (extraWaitsLeft > 0) {
        extraWaitsLeft -= 1;
        dbg(`extra wait (left=${extraWaitsLeft}): long sleep + re-scroll`);
      ...
      // 已加载完、没 retry、兜底用尽、仍无新增 → 大概率真到底/X 不再返回更多
      stagnant += 1;
      dbg(`stagnant=${stagnant} (no loading, no retry, extra waits exhausted)`);
      if (stagnant >= maxStagnantRounds) { dbg(`break: stagnant>=maxStagnantRounds`); break; }
    } else {
      stagnant = 0;
    }
    ...
    if (items.length === 0 && attempt < 2) {
      dbg(`early wait (items=0, attempt<2)`);
      await sleep(scrollDelay);
      continue;
    }

    const scrollResult = await proxy.scroll(targetId, { direction: 'bottom' }).catch(e => ({ __err: e?.message || String(e) }));
    dbg(`scroll bottom → ${scrollResult?.__err ? 'ERR: ' + scrollResult.__err : 'ok'}`);
    await sleep(scrollDelay);
  }

  dbg(`return items=${items.length} (limit=${limit})`);
...
export default {
  ...
  async _extractList(proxy, targetId, limit, url) {
    ...
    // 优先走 store 路径:X 自己已经通过 ListLatestTweetsTimeline GraphQL 把 entries 加载到 redux store,
    // 读它比爬 DOM 更完整、且自带长推全文 + idempotent。DOM 路径留作 fallback(store 挖不到 / timeline 没加载时)。
    let items = [];
    let fetchStrategy = 'dom';
    let entriesInStore = null;
    let missingIdCount = null;
    let apiError = null;

    if (listId) {
      const api = await evalJson(proxy, targetId, buildListApiExtractJS(listId, limit)).catch(e => ({ error: 'eval_throw: ' + (e && e.message || String(e)) }));
      entriesInStore = api?.entriesInStore ?? null;
      missingIdCount = api?.missingIdCount ?? null;
      apiError = api?.error || null;
      if (api && Array.isArray(api.items) && api.items.length > 0) {
        items = api.items;
        fetchStrategy = api.strategy || 'store';
      }
    }
