// X adapter — 登录态 Chrome 下抽 list / profile / status 三种 page type:
//   - list / profile timeline 从 X 的 redux store 直接读(见 BROWSER_TIMELINE_STORE_JS):
//     state.urt.<timelineKey> 拿 entries,state.entities.tweets/users 拿实体。不爬 DOM。
//     长推全文(note_tweet 已并入 full_text)、repost 链(retweeted_status / user)自带。
//   - status(单推详情页)仍走 DOM selector(给 scripts/fetch-article.mjs 用)
