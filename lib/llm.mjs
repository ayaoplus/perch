...
export async function runPromptWithTools(prompt, opts = {}) {
  ...
  const _maxRetriesRaw = Number(process.env.PERCH_LLM_MAX_RETRIES ?? DEFAULT_MAX_RETRIES);
  const maxRetries = opts.maxRetries ?? (Number.isFinite(_maxRetriesRaw) && _maxRetriesRaw >= 0
    ? Math.floor(_maxRetriesRaw)
    : DEFAULT_MAX_RETRIES);
