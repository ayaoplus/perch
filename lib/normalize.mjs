...
// Block 标题行的"完整指纹":`## ` 开头 + 同行包含 `[source](<http url 含 /status/<id>>)`。
// 这是 formatTweet 生成每个 block 时的必有结构(DESIGN §5),tweet 正文里即使写了 `## ...`
// 也几乎不会同时带 `[source](https://.../status/NNN)`,因此不会被误判。
//
// 刻意不单独匹配 `## `:长推正文可能包含作者自己写的 markdown heading(例如 "## 结论"),
// 之前用 `/^## /m` 会把这行当成新 block 起点,造成 collect 重写时把单条 tweet 切成多块。
const BLOCK_HEADING_RE = /^## [^\n]*\[source\]\(https?:\/\/[^)\n]*\/status\/\d+\)/m;
const BLOCK_HEADING_SPLIT_RE = /(?=^## [^\n]*\[source\]\(https?:\/\/[^)\n]*\/status\/\d+\))/m;

/**
 * 把已有 raw 文件内容拆成 `{ header, blocks }`。
 *   - header:文件顶部到第一个 block 标题**之前**的那段(文件标题 + Sources 列表 + 分隔线)
 *   - blocks:每个合法 block 标题开始到下一个 block 标题之前的整段字符串
 *
 * **block 标题判定**:`## ` + 同一行有 `[source](https://.../status/<digits>)`。单独匹配 `## `
 * 会误伤长推正文里的 markdown heading(例如 `## 结论`),把一条推文误切成两块。
 *
 * 如果文件里一个合法 block 标题都没有(刚建 / 空文件 / 只有 header),blocks 为 `[]`。
 *
...
export function splitRawBlocks(content) {
  const firstHeading = content.search(BLOCK_HEADING_RE);
  if (firstHeading < 0) {
    return { header: content, blocks: [] };
  }
  const header = content.slice(0, firstHeading);
  const tail = content.slice(firstHeading);
  const blocks = tail.split(BLOCK_HEADING_SPLIT_RE).filter(Boolean);
