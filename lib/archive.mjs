...
export async function archive(topic, opts = {}) {
  ...
  if (months.length === 0) {
    ...
    return { topic: topic.slug, dryRun, months: [], moves: [], totalMoves: 0, totalSkipped: 0 };
  }
  ...
  for (const month of months) {
    ...
    if (!dryRun) {
      const { skipped } = await executePlan(plan, log);
      totalSkipped += skipped;
      if (skipped > 0) {
        log(`   ! ${skipped} file(s) skipped (target already exists in archive)`);
      }
    }
  }

  const totalMoved = dryRun ? 0 : allMoves.length - totalSkipped;
  log(`total: ${dryRun ? `${allMoves.length} would be moved` : `${totalMoved} moved, ${totalSkipped} skipped`}`);
  return {
    topic: topic.slug,
    dryRun,
    months,
    moves: allMoves,
    totalMoves: dryRun ? allMoves.length : totalMoved,
    totalSkipped,
  };
}
 ...
 * 返回 { moved, skipped } 计数。
 */
export async function executePlan(plan, log) {
  let moved = 0;
  let skipped = 0;
  for (const move of plan.moves) {
    ...
    if (targetExists) {
      if (log) log(`   skip (target exists): ${move.to}`);
      skipped++;
      continue;
    }
